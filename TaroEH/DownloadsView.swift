...
struct OfflineReaderView: View {
    ...
    let startIndex: Int?
    init(gallery: Gallery, startIndex: Int? = nil) {
    ...
    var body: some View {
        if pages.isEmpty {
            ContentUnavailableView("没有可读页面", systemImage: "photo")
        } else {
            let saved = reading.page(for: gallery)
            let requested = startIndex ?? saved
            SharedReaderView(gallery: gallery, title: gallery.title, pageCount: pages.count, initialIndex: min(requested, max(0, pages.count - 1)), onIndexChange: { index in
                reading.save(gallery: gallery, pageIndex: index)
            }, onPageAppear: { _ in }) { i, fit, scale in
                ReaderPageImage(url: pages[i], referer: nil, pageNumber: i + 1, status: .loaded, fit: fit, scale: scale, onRetry: {}, onAutoRetry: {})
